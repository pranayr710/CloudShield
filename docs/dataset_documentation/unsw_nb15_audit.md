...
## Finding 6 — 7.07% of rows are label-ambiguous: identical features, different class ⚠️

Found by the verification suite, not by inspection.

After deduplication, zero fully-identical rows remain. But once `attack_cat`,
`label` and `sloss` are removed to form the feature matrix, **2,445 groups of rows
become identical** — 11,506 rows, 7.07% of the dataset.

The breakdown is what makes this interesting:

| Check | Result |
|---|---|
| Feature-identical groups | **2,445** covering 11,506 rows (7.07%) |
| Groups that **disagree on `attack_cat`** | **2,445 — every single one** |
| Groups that disagree on `sloss` | **0 — none** |

So these are not residual duplicates. They are genuinely distinct observations
that **UNSW-NB15 itself labels inconsistently**: identical flow statistics
assigned to different attack families by the dataset authors.

**Consequences, and they differ by track:**

- **Classification** — this is an irreducible accuracy ceiling. Roughly 7% of rows
  cannot be classified correctly by *any* model, because the same feature vector
  appears with two or more different labels. A model reporting 97% accuracy on
  this dataset should be treated with suspicion. It also partly explains the
  expected `Analysis`/`Backdoor`/`Exploits` confusion: some of that confusion is
  in the ground truth, not the model.
- **Regression** — completely unaffected. Feature-identical rows always share the
  same `sloss`, so the regression target stays perfectly well-defined. Worth
  stating explicitly, because it means the two tracks have different data-quality
  ceilings.

**Decision: report it, do not deduplicate it.** Removing these rows would mean
arbitrarily choosing one label over another, discarding real information about how
ambiguous the labelling is. `tests/test_preprocessing.py` pins the behaviour, so a
future change cannot silently convert this into genuine leakage: the test asserts
that every colliding group disagrees on the class label and that none disagrees on
the regression target.

**Viva answer:** "Our classification accuracy has a hard ceiling around 93%
because 7% of the flows are labelled inconsistently in the source data. We
measured it rather than assuming our model was at fault."

---

## Finding 7 — Categorical columns need level folding, not naive one-hot
...
## Finding 8 — Structural zero-blocks are informative, not missing data
...
## Finding 9 — `label` is a deterministic function of `attack_cat`
